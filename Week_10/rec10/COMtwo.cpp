...
int main(){
    ...
    file_out1.open("summary.txt");
    file_out2.open("error_log.txt");
    file_in.open("coordinates2.txt");
    string dALL,d2,d3;
    int size = 100;
    string arr[size];
    bool allValid = true;
    ...
    while(getline(file_in, dALL)){
        ...
        for(int i = 0; i < 3; i++){
            if (validateDouble(arr[i]) == false){
                file_out2 << "Ivalid Entry: ";
                file_out2 << arr[0]<< "," << arr[1] <<"," << arr[2] << "," <<endl;
                allValid = false;
                count = count - 1;
            }else if(validateDouble(arr[0]) == true && validateDouble(arr[1]) == true && validateDouble(arr[2]) == true){
                if(i == 0){
                    x += stod(arr[i]);
                }else if(i == 1){
                   y += stod(arr[i]); 
    ...
    file_out1 << "Center of Mass:" <<endl;
    file_out1 << "X: " << comx <<endl;
    file_out1 << "Y: " <<comy <<endl;
    file_out1 << "Z: " <<comz <<endl;
    file_out1 << endl;
    file_out1 << "Number of valid entries: " << count << endl;
    
        file_out1 << "Process completed successfully!" << endl;
    }else{
        file_out1 << "Process completed successfully!" << endl;
    }
    return 0;
